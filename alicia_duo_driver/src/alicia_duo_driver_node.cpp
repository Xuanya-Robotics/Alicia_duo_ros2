#include "alicia_duo_driver_node.hpp"
...
// In AliciaDuoDriverNode::AliciaDuoDriverNode()
...
{
    // Publishers
    joint_state_pub_ = this->create_publisher<alicia_duo_driver::msg::ArmJointState>("/arm_joint_state", 10);
    array_pub_ = this->create_publisher<std_msgs::msg::Float32MultiArray>("/servo_states_main", 10);

    // Subscribers
    joint_command_sub_ = this->create_subscription<alicia_duo_driver::msg::ArmJointState>(
        "/arm_joint_command", 10, std::bind(&AliciaDuoDriverNode::joint_command_callback, this, std::placeholders::_1));
...
void AliciaDuoDriverNode::joint_command_callback(const alicia_duo_driver::msg::ArmJointState::SharedPtr msg)
{
    if (!communicator_->is_connected()) return;

    std::vector<double> joint_angles = {msg->joint1, msg->joint2, msg->joint3, msg->joint4, msg->joint5, msg->joint6};
    ...
    // --- Prepare and Send Servo Control Frame (0x04) ---
    {
        ...
        for (int i = 0; i < servo_count_; ++i) {
            ...
            // Check if servo index is valid for the map
            if (static_cast<size_t>(i) < joint_to_servo_map_index_.size()) {
                ...
                // *** ADDED THE CRITICAL SAFETY CHECK HERE ***
                if (static_cast<size_t>(joint_idx) < joint_angles.size()) {
                    hw_val = rad_to_hardware_value(joint_angles[joint_idx] * direction);
                    // RCLCPP_WARN(this->get_logger(), "hw_val %d for servo %d (joint %d): %d", hw_val, i, joint_idx, hw_val);
                } else {
                    RCLCPP_WARN(this->get_logger(), "Joint index %d from map is out of bounds for servo %d", joint_idx, i);
                    // hw_val remains at its default value
                }
            } else {
                RCLCPP_WARN(this->get_logger(), "Servo index %d is out of bounds for joint_to_servo_map", i);
                // hw_val remains at its default value
            }
            
            size_t frame_idx = 3 + i * 2;
            servo_frame[frame_idx] = hw_val & 0xFF;      // Low byte
            servo_frame[frame_idx + 1] = (hw_val >> 8) & 0xFF; // High byte
        }

        servo_frame[frame_size - 1] = FRAME_END_BYTE;
        servo_frame[frame_size - 2] = calculate_checksum(servo_frame); // Calculate and set checksum
    ...
    // --- Prepare and Send Gripper Control Frame (0x02) ---
    {
        ...
        uint16_t gripper_hw_val = rad_to_hardware_value_grip(msg->gripper);
        gripper_frame[4] = gripper_hw_val & 0xFF;      // Low byte
        gripper_frame[5] = (gripper_hw_val >> 8) & 0xFF; // High byte

        gripper_frame[7] = FRAME_END_BYTE;
        gripper_frame[6] = calculate_checksum(gripper_frame); // Calculate and set checksum
